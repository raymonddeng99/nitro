...
void I2C_GPIO_Init(void) {
	...
	// Set both GPIO pins to alternative function mode (AF4)
	...
	// Set both pins to have a open drain output type
...
void I2C_Initialization(void){
	...
	// 1. Set up the clock for I2C in the RCC registers.
	RCC->APB1ENR1 |= RCC_APB1ENR1_I2C1EN;

	// 1a. Enable the clock for I2C1 in the peripheral clock enable register.
	RCC->CCIPR &= ~(RCC_CCIPR_I2C1SEL);

	// 1b. Set the system clock as the clock source for I2C1 in the peripherals independent clock configuration register.
	...
	//		Clear the bits so I2C1 does not remain in a reset state.
	RCC->APB1RSTR1 &= ~(RCC_APB1RSTR1_I2C1RST);
	


	// 2. Configure I2C registers for communication with the temperature sensor.
	...
	// System clock freq = 80 MHz
	// PRESC = 7

	I2C1->TIMINGR &= ~(I2C_TIMINGR_PRESC);
	I2C1->TIMINGR |= (0b0111 << I2C_TIMINGR_PRESC_POS);

	// t_PRESC = 0.1 microsec

	// t_SCLDEL = 1 micro sec = (SCLDEL + 1) * t_PRESC
	// 	=> SCLDEL = (1 micro sec / 0.1 micro sec) - 1 = 9

	I2C1->TIMINGR &= ~(I2C_TIMINGR_SCLDEL);
	I2C1->TIMINGR |= (0b1001 << I2C_TIMINGR_SCLDEL_POS);

	// t_SDADEL = 1.250 micro sec = (SDADEL + 1) * t_PRESC
 	// => SDADEL = 11.50 micro sec ~ 12 micro sec 1100

 	I2C1->TIMINGR &= ~(I2C_TIMINGR_SDADEL);
	I2C1->TIMINGR |= (0b1100 << I2C_TIMINGR_SDADEL_POS);

	// t_SCLH = 4 micro sec = (SCLH + 1) * t_PRESC
 	// => SCLH = 39 micro sec = 0b00100111

 	I2C1->TIMINGR &= ~(I2C_TIMINGR_SCLH);
 	I2C1->TIMINGR |= (0b00100111 << I2C_TIMINGR_SCLH_POS);

 	// t_SCLL = 4.7 micro sec = (SCLL + 1) * t_PRESC
 	// => SCLL = 46 micro sec = 0b00101110

 	I2C1->TIMINGR &= ~(I2C_TIMINGR_SCLL);
 	I2C1->TIMINGR |= (0b00101110 << I2C_TIMINGR_SCLL_POS);


	// 3. Set your 'own address' in the own address registers.

	// 3a. Set own address to 7-bit mode.
	I2C1->OAR1 &= ~(I2C_OAR1_OA1MODE);

	// 3b. Write the address you want to use.
	I2C1->OAR1 |= OwnAddr;

	// 3c. Enable own address.
